...
async fn test_cli_bad_method() {
    ...
    let connection = ledger_signer::ledger::get_test_connection().await.unwrap();
    let result = run_cli(cursor, connection).await;

    assert!(result.is_err());
    let error = result.unwrap_err();
    assert!(error.to_string().contains("Invalid method: bad_method"));
}
...
async fn test_cli_sign() {
    ...
    let connection = ledger_signer::ledger::get_test_connection().await.unwrap();
    let (result, _) = tokio::join!(run_cli(input, connection), async {
...
async fn test_cli_get_public_key() {
    ...
    let connection = ledger_signer::ledger::get_test_connection().await.unwrap();
    let result = run_cli(input, connection).await.unwrap();
